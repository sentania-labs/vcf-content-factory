...
| `content/` | Authored factory output: supermetrics, views, dashboards, alerts, MPs. `content/sdk-adapters/*` are independent git repos (gitignored, bootstrap-cloned). `content/migrator/` is the `sentania-labs/vcf-cf-migrator` clone (gitignored, cloned by hand; spec `knowledge/designs/content-migrator-v1.md`), with the real export corpus under its gitignored `corpus/`. |
