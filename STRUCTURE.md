...
| `content/` | Authored factory output: supermetrics, views, dashboards, alerts, MPs. `content/sdk-adapters/*` are independent git repos (gitignored, bootstrap-cloned). `content/migrator/` is the `sentania-labs/vcf-cf-migrator` clone (gitignored, cloned by hand), with the real export corpus under its gitignored `corpus/`. Its spec and review records moved into that repo after v0.3.0; `knowledge/designs/content-migrator-v1.md` is a pointer to them and says what stayed here. |
...
| `src/` | The `vcfcf_core` library (pure parse/walk/render/build code, the only thing the root `pyproject.toml` packages, published as `vcf-cf-tooling-core` on `core-v*` tags) and the ten `vcfcf_*` framework Python packages (`src/vcfcf_alerts/` … `src/vcfcf_symptoms/`). **Only the `tooling` agent edits these** (RULE-013 gate applies). Directories moved under `src/` in reorg v2 phase 1 (as `vcfops_*`); renamed to `vcfcf_*` in M1 (2026-09-14). `src/vcfops_*/` are one-release compatibility shims that forward to the new names with a deprecation line; removed next release. Ambient `PYTHONPATH=src`. |
...
| `third_party/` | Redistributed third-party content items — deliberately **not** under `reference/`: it is content-adjacent (shipped by us, machine-routed by `vcfcf_packaging`'s release builder), authored by others, attribution required. |
