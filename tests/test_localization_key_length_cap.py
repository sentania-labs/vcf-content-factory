...
``vcfcf_managementpacks.sdk_builder`` and the dormant twin in
``vcfcf_dashboards.render``) derived a ``content.properties`` /
...
class TestSdkBuilderCap:
    ...
    def test_short_key_passes_through_unchanged(self):
        from vcfcf_managementpacks.sdk_builder import _attribute_to_localization_key
    ...
    def test_two_long_keys_sharing_64_char_prefix_dont_collide(self):
        from vcfcf_managementpacks.sdk_builder import _cap_localization_key
...
class TestDashboardsRenderCapTwin:
    """The dormant twin in vcfcf_dashboards.render must cap identically."""

    def test_repro_key_maps_to_capped_unique_key(self):
        from vcfcf_dashboards.render import (
            _attribute_to_localization_key,
    ...
    def test_two_long_keys_sharing_64_char_prefix_dont_collide(self):
        from vcfcf_dashboards.render import _cap_localization_key
...
class TestBuildTimeGuard:
    ...
    def test_guard_trips_on_synthetic_over_long_key(self, monkeypatch, tmp_path: Path):
        ...
        # vcfcf_dashboards.render.render_views_xml, which needs a real
