...
current behavior (`src/vcfcf_core/packaging/assembly.py::_build_dashboard_dropin_zip`).
...
`src/vcfcf_core/packaging/assembly.py::_build_dashboard_dropin_zip`). No
...
1. **`Dashboard.zip`** — no changes. The builder's current code at
   `src/vcfcf_core/packaging/assembly.py::_build_dashboard_dropin_zip` (which
