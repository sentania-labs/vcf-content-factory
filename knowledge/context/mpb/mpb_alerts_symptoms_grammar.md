...
### Type / SubType code table

`type` and `subType` are integer codes whose meaning is platform-owned (not in describe.xml). VCF-CF should:

1. Maintain an enum (extracted by inspection of the platform's source or via a one-time observation of which alerts the platform classifies as which) — mongodb uses (15, 22) for all alerts; vSphere uses (15, 16, 20) × (6, 18, 19, 20, 21, 22, 28, 29).
2. Default generated alerts to (15, 22) when category is unknown — that's the "general performance / availability" combination mongodb defaults to.
3. Expose the type/subType taxonomy to users authoring custom alerts, with descriptive labels.
