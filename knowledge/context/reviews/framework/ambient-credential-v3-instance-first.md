...
1. **Bytecode claim — VERIFIED TRUE.** `javap -c` on `vrops-adapters-sdk-2.2.jar`:
   `AdapterCredentialConfig.getPassword()` = `invokestatic com/vmware/vcops/security/Crypt.getDefaultCrypt` → `getfield password` → `invokevirtual Crypt.decrypt` → `areturn`. It is **not** a bare field accessor. `getUserName()` **is** a bare `getfield userName`. Matches `context/api-surface/per-instance-suiteapi-credential-contract.md` §7.
...
**Report:** context/reviews/framework/ambient-credential-v3-instance-first.md
