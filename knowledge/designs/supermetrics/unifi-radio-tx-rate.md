# [VCF Content Factory] UniFi Radio TX Rate

- **Type:** supermetric
- **Slug:** unifi-radio-tx-rate
...
- Turn the lifetime counter `Traffic|tx_bytes` on `UniFiRadio` into a current rate in bytes/s, so dashboards can show what the port, radio or uplink is doing now.
