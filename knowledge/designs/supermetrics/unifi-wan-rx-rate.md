# [VCF Content Factory] UniFi WAN RX Rate

- **Type:** supermetric
- **Slug:** unifi-wan-rx-rate
...
- Turn the lifetime counter `Traffic|rx_bytes` on `UniFiWanInterface` into a current rate in bytes/s, so dashboards can show what the port, radio or uplink is doing now.
...
An adapter change that emits the `-r` fields (`wan1.rx_bytes-r`); the describe.xml unit bytes/s on this key is a label bug would give a true
