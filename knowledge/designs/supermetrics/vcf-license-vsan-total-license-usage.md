# vSAN Total License Usage (TiB)

- **Type:** supermetric
- **Slug:** vcf-license-vsan-total-license-usage
- **Authored YAML:** supermetrics/vcf_license_vsan_total_license_usage.yaml
- **Date:** 2026-08-26
- **Status:** drafted
...
- Total vSAN capacity licenses consumed (TiB per the source dashboard label), from the
  platform license usage objects filtered to any `vSAN*` product.
- Feeds License Overview (vSAN Consumed TiB), vSAN Usage Over Time trend, VCF Licensing
  Overtime month table.
- Formula unchanged from source.

## Spec

**Name:** `[VCF Content Factory] vSAN Total License Usage (TiB)` (RULE-006 prefix, literal brackets, one space).
Source name: `[VCF Consumption Overview v2] vSAN Total License Usage`.
...
- `startsWith vSAN` matches every vSAN edition (ro818 has `vSAN Enterprise`). Intentional.
