# VCF Total License Capacity (cores)

- **Type:** supermetric
- **Slug:** vcf-license-vcf-total-license-count
- **Authored YAML:** supermetrics/vcf_license_vcf_total_license_count.yaml
- **Date:** 2026-08-26
- **Status:** drafted
...
- Total VCF core licenses owned (sum of cost-unit limits on the VCF license keys).
- Feeds License Overview (Total VCF License Capacity) and the VCF Licensing Overtime table.
- Renamed from "Count" to "Capacity" so it reads the same as its vSAN sibling and the
  dashboard label; formula unchanged.

## Spec

**Name:** `[VCF Content Factory] VCF Total License Capacity (cores)` (RULE-006 prefix, literal brackets, one space).
Source name: `[VCF Consumption Overview v2] VCF Total License Count`.
...
- `CostUnitLimit` is a property on 8.18. The source addressed it with `metric=` and it
  worked; the DSL accepts property keys in the `metric=` slot (pitfall 5). Keep as is.
