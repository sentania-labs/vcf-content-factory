# VCF Total License Usage (%)

- **Type:** supermetric
- **Slug:** vcf-license-vcf-total-license-usage-percent
- **Authored YAML:** supermetrics/vcf_license_vcf_total_license_usage_percent.yaml
- **Date:** 2026-08-26
- **Status:** drafted
...
- VCF consumed cores as a percentage of licensed cores. Feeds License Overview
  (VCF License Usage %) and the VCF Licensing Overtime table (unit percent).

## Spec

**Name:** `[VCF Content Factory] VCF Total License Usage (%)` (RULE-006 prefix, literal brackets, one space).
Source name: `[VCF Consumption Overview v2] VCF Total License Usage Percent`.
...
```
(sum(${adaptertype=VMWARE_INFRA_HEALTH, objecttype=LicenseUsage, metric=Assets|TotalUsage, depth=10, where="ProductId startsWith vSphere 8 Enterprise Plus for VCF"}) / sum(${adaptertype=VMWARE_INFRA_HEALTH, objecttype=LicenseUsage, metric=CostUnitAttributes|CostUnitLimit, depth=10, where="ProductId startsWith vSphere 8 Enterprise Plus for VCF"}) ) * 100
```

**Rebuild formula:**

```
(${this, metric=Super Metric|@supermetric:"[VCF Content Factory] VCF Total License Usage (cores)"}
 / ${this, metric=Super Metric|@supermetric:"[VCF Content Factory] VCF Total License Capacity (cores)"}) * 100
```
...
- `[VCF Content Factory] VCF Total License Usage (cores)`
- `[VCF Content Factory] VCF Total License Capacity (cores)`
