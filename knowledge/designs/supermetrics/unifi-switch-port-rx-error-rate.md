# [VCF Content Factory] UniFi Switch Port RX Error Rate

- **Type:** supermetric
- **Slug:** unifi-switch-port-rx-error-rate
- **Authored YAML:** none (not expressible, see below)
- **Assigned to:** `unifi_controller` `UniFiSwitchPort`
- **Date:** 2026-09-25
...
- Turn the lifetime counter `Traffic|rx_errors` on `UniFiSwitchPort` into a current errors per interval, so dashboards can show what the port, radio or uplink is doing now.
...
Charts of `Traffic|rx_errors` are relabelled "cumulative since device counter
...
rate; the value is never presented as current throughput, and error counts are labelled lifetime and uncolored. Widgets
changed: unifi-summary-switch-port S3 (tile labelled lifetime), unifi-switch-ports view (column labelled lifetime, uncolored, removed from the summary row), unifi-switching-poe W5 (tab labelled lifetime).
...
An adapter change that emits the `-r` fields (no `-r` error field is known; an adapter-side delta between cycles would be needed) would give a true
