# [VCF Content Factory] UniFi Switch Port RX Rate

- **Type:** supermetric
- **Slug:** unifi-switch-port-rx-rate
...
- Turn the lifetime counter `Traffic|rx_bytes` on `UniFiSwitchPort` into a current rate in bytes/s, so dashboards can show what the port, radio or uplink is doing now.
...
An adapter change that emits the `-r` fields (port_table `rx_bytes-r`) would give a true
