# [VCF Content Factory] UniFi Switch Port TX Error Rate

- **Type:** supermetric
- **Slug:** unifi-switch-port-tx-error-rate
...
- Turn the lifetime counter `Traffic|tx_errors` on `UniFiSwitchPort` into a current errors per interval, so dashboards can show what the port, radio or uplink is doing now.
...
Charts of `Traffic|tx_errors` are relabelled "cumulative since device counter
...
changed: unifi-summary-switch-port S3 (tile labelled lifetime), unifi-switch-ports view (column labelled lifetime, uncolored).
