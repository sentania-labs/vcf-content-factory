# [VCF Content Factory] UniFi Switch Port TX Rate

- **Type:** supermetric
- **Slug:** unifi-switch-port-tx-rate
...
- Turn the lifetime counter `Traffic|tx_bytes` on `UniFiSwitchPort` into a current rate in bytes/s, so dashboards can show what the port, radio or uplink is doing now.
...
changed: unifi-summary-switch-port S5, unifi-switching-poe W7.
...
An adapter change that emits the `-r` fields (port_table `tx_bytes-r`) would give a true
