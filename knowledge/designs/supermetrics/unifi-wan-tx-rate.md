# [VCF Content Factory] UniFi WAN TX Rate

- **Type:** supermetric
- **Slug:** unifi-wan-tx-rate
- **Authored YAML:** none (not expressible, see below)
- **Assigned to:** `unifi_controller` `UniFiWanInterface`
- **Date:** 2026-09-25
...
- Turn the lifetime counter `Traffic|tx_bytes` on `UniFiWanInterface` into a current rate in bytes/s, so dashboards can show what the port, radio or uplink is doing now.
...
Charts of `Traffic|tx_bytes` are relabelled "cumulative since device counter
...
changed: unifi-summary-wan-interface S6, unifi-wan-interfaces view (columns labelled lifetime totals).
...
An adapter change that emits the `-r` fields (`wan1.tx_bytes-r`); the describe.xml unit bytes/s on this key is a label bug would give a true
