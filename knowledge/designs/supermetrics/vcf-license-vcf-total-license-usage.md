# VCF Total License Usage (cores)

- **Type:** supermetric
- **Slug:** vcf-license-vcf-total-license-usage
- **Authored YAML:** supermetrics/vcf_license_vcf_total_license_usage.yaml
- **Date:** 2026-08-26
- **Status:** drafted
...
- Total VCF core licenses consumed, as reported by the platform's own license usage
  objects, filtered to the `vSphere 8 Enterprise Plus for VCF` product.
- Feeds the License Overview scoreboard (VCF Consumed Cores), the VCF License Usage Over
  Time trend and the VCF Licensing Overtime month table.
- Formula unchanged from source; only the name, description and prefix change.

## Spec

**Name:** `[VCF Content Factory] VCF Total License Usage (cores)` (RULE-006 prefix, literal brackets, one space).
Source name: `[VCF Consumption Overview v2] VCF Total License Usage`.
...
- Source description says "vSAN" (copy-paste); rebuild description must say VCF.
