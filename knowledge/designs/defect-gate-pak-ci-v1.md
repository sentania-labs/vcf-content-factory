...
- **SDK-pointer-release cascade is ALREADY done.** `_gate_publish`
  (`src/vcfcf_packaging/publish.py:759`) detects an SDK adapter headline
...
2. **Each pak's CI gates the tag build.** A step in
   ...
   `ci/defect_gate.py`** (a copy of `src/vcfcf_packaging/defects.py`) into
   each repo.
...
- `tooling` → the `defects.py` standalone entrypoint + tests
  (`src/vcfcf_packaging/`). Triggers the **RULE-013 `framework-reviewer`
...
- `sdk-adapter-author` → propagate to the 4 pak repos + template (direct
  ...
  `src/vcfcf_packaging/defects.py`) into each repo. Post-merge only (main
  must carry the standalone `defects.py` first).

## Acceptance (negative proof)

- `python3 src/vcfcf_packaging/defects.py --pak synology --registry knowledge/context/defects.md`
  → exit 2, names DEF-001 (the same refusal the package CLI gives).
- A bare copy of `defects.py` run outside the package (no
  `vcfcf_packaging` on `PYTHONPATH`) gives the identical result —
