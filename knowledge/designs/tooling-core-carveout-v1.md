...
|---|---|---|
| 1 | Scaffold, contract test, wheel build in CI on `core-v*`. Plus the modules with no leaks at all: `dashboards/yaml_utils`, `supermetrics/crossref`, `supermetrics/reverse`, `symptoms/loader`, `alerts/loader`, `alerts/render`, `reports/render`, `packaging/deps`, `packaging/release_types`, `packaging/template_version` | none; this PR proves the mechanism |
