# Design note — synology adapter v2 migration
...
- Port synology from v1 (aria-ops-core) to framework v2: VcfCfAdapter +
  com.vcfcf.adapter.spi roles. REST-only adapter — no JAX-WS/SOAP
  involvement, so no transport rewrite needed beyond the SPI port.
