...
UUIDs are part of the contract for super metrics, views, dashboards, and reports. Generate stable UUID on first `validate`, never touch after. Cross-references resolve to literal `sm_<uuid>` / `viewDefinitionId` strings. See `knowledge/context/authoring/uuids_and_cross_references.md`.
