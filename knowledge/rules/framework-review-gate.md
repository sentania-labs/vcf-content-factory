...
No change to `src/vcfcf_*/` (the framework Python — loaders, renderers,
builders, CLIs) merges to `main` until the **`framework-reviewer`** agent
has reviewed it and returned **APPROVE** (zero BLOCKING).

The flow:

1. `tooling` makes the `src/vcfcf_*/` change and reports it (the only agent
   that edits `src/vcfcf_*/`).
2. **Before the PR is opened**, the orchestrator spawns
   `framework-reviewer`. Scope is **blanket** — *every* `src/vcfcf_*/` diff,
...
- **The reviewer is read-only and independent.** It never edits
  `src/vcfcf_*/`; it hands findings back to `tooling`. A reviewer that edits
...
- **CI reminds, does not gate.** `scripts/check_framework_review.sh` warns
  (non-blocking) when a PR touches `src/vcfcf_*/` with no
