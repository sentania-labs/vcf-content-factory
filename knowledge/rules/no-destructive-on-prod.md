...
**If violated:** Built-in pak removal leaves the instance in an unrecoverable state. VCF Ops 9.0.2 does not enforce `isUnremovable` server-side — the flag is advisory-only, the API will accept and execute the call. Recovery requires cluster restart or snapshot restore. See `knowledge/lessons/pak-isunremovable-vendor-bug.md`.
