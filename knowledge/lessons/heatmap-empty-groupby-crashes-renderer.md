...
`src/vcfcf_dashboards/render.py` — the Heatmap renderer — previously emitted
...
- `src/vcfcf_dashboards/render.py` — Heatmap `else: group_by = {}` replaced
  with self-grouping block (lines ~1197–1240); `_alert_list_widget` updated
  to handle `pin_to_world`.
- `src/vcfcf_dashboards/loader.py` — `AlertListConfig` dataclass: new
