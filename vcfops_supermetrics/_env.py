...
`VCFOpsClient.from_env()` / `VCFOpsCustomGroupClient.from_env()` gets
credentials automatically, removing the need for agents to shell-
source `.env` before invoking Python (which triggers Claude Code's
shell-injection prompt).
"""
