...
8.18 instance), and a single instance never reports the whole union.
  ...
  - key absent from the live response: entry is RETAINED unchanged, and the
    refresh emits a WARN naming every retained key so a reviewer can see
    what the live instance no longer reports

A key is only ever removed when the caller explicitly asks for it:
``refresh(..., prune=True)`` (CLI: ``refresh-describe --prune``).  Prune
...
Top-level keys the refresh does not own (``merged_from``, ``merge_note``,
anything else hand-written) are carried through unchanged.  ``fetched_at``
and ``source`` always describe the most recent refresh.
...
(value changed), retained-but-absent, and pruned.
"""
...
class DescribeCache:
    ...
    def refresh(
        ...
        are retained (and listed in a WARN) unless ``prune=True``, in which
        case they are removed.  See the module docstring, "Merge semantics".
        ...
        # Carry through top-level keys we do not own (merged_from,
        # merge_note, ...); overwrite only the ones this refresh produces.
...
def _merge_section(
    ...
    unless ``prune`` is True.  Returns the merged dict and the stats.
    """
    stats = MergeStats([], [], [], [], [])
    ...
    for key, meta in live.items():
        ...
        merged[key] = meta
    for lst in (stats.added, stats.updated, stats.unchanged, stats.retained, stats.pruned):
        lst.sort()
    return merged, stats
