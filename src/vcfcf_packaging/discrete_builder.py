...
Usage::
    ...
    python3 -m vcfcf_packaging build-discrete <content-type> <item-name-or-path>
...
from vcfcf_symptoms.loader import SymptomDef, load_dir as load_symptom_dir
...
def _expand_sm_crossrefs(
    ...
    Thin adapter over ``vcfcf_common.dep_walker.expand_sm_crossrefs`` (the
    ...
    """
    from vcfcf_common.dep_walker import expand_sm_crossrefs
...
def _generate_discrete_readme(
    ...
    lines += [
        ...
        "",
        "---",
        "_Generated by vcfcf_packaging. Part of the VCF Content Factory framework._",
    ]
    return "\n".join(lines) + "\n"
...
def build_discrete(
    ...
    *,
    ...
    if ct == "supermetric":
        ...
        from vcfcf_common.dep_walker import collect_deps as _collect_deps
...
def _load_all_reports(
    ...
    from vcfcf_reports.loader import load_dir
    results = []
    for proj_root in (extra_search_dirs or []):
        d = proj_root / "reports"
        if d.exists():
