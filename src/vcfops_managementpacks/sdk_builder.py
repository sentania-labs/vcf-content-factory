...
def _write_outer_pak(
    ...
    with zipfile.ZipFile(pak_path, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        # attribute at all). Our own ViewDef fragments match that shape too as
        # of DEF-018 — render.py no longer emits localizationKey="title"/"desc"
        # (a VCF Ops 8.18 hard-reject: the server-side view importer rejects a
        # localizationKey with no backing content.properties bundle in the
        # same import unit, and our content-import zips never shipped one).
        # We still emit a matching resources/content.properties whenever a
        # view is embedded — spec A3 wants the resources/ subdirectory
...
def _validate_localization_key_contract(views: list, sm_scope: Optional[List[Path]] = None) -> List[str]:
    ...
    Dormant on the Title/Description path since DEF-018: the XML no longer
    emits localizationKey on <Title>/<Description> (render.py's
    _render_view_def_fragment drops it unconditionally), so step 3's XML
    scan finds zero keys there and this guard trips nothing for that class.
    Kept as a trip-wire in case Title/Description localizationKeys are ever
    re-enabled — same rationale as the column-case dormant note at
    render.py:216-224.
    """
    import re as _re
