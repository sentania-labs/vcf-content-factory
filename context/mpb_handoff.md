...
7. **The substitution/expression language is `${...}` templates** with explicit namespaces: `${configuration.X}`, `${authentication.credentials.X}`, `${authentication.basic}`, `${authentication.session.X}`, `${requestParameters.X}`, `${@@@MPB_QUOTE_BODY <jsonpath> @@@MPB_QUOTE}` for response navigation. Same language across requests, headers, expressions, identifiers.
...
9. **Pak signing is broken in instructive ways.** Single self-signed VMware cert signs everything (Broadcom-internal + marketplace re-signed third-party). Cert **expired 2026-01-03** but paks dated 2025-12-30 still ship with it; appliance presumably skips validity checks. SHA-1 hashes + sha1WithRSAEncryption + RSA exponent 3. Community/dev paks ship UNSIGNED (zero-byte `signature.cert`). VCF-CF needs to decide signing posture; the unsigned/admin-override path is the path of least resistance. See `analysis/pak-signing-chain.md`.
...
| Auto-namespaced ResourceKind: `<adapterKind>_<label-slugified>` (e.g., `mpb_ubiquiti_unifi_unifi___clients`) | computed at transform time |
...
| Substitution language | `${configuration.X}`, `${authentication.credentials.X}`, `${authentication.basic}`, `${authentication.session.X}`, `${requestParameters.X}`, `${@@@MPB_QUOTE_BODY <jsonpath> @@@MPB_QUOTE}` | Uniform across all expression contexts |
...
- ❌ Value transforms beyond BASE64 (URL encode, regex extract, JSONPath, XPath, concatenation, arithmetic, conditional)
...
### Adopt aria-ops-core SPI for vcfcf-adapter-base.jar

The Pass 3 finding (BlueMedora marketplace adapters use it) has been triple-confirmed:
1. BlueMedora-derived marketplace MPs (mongodb, mysql, postgresql, oracle, etc.) — Pass 3
2. The MPB runtime engine itself uses it (`com.vmware.mpb.MPBAdapter extends UnlicensedAdapter` from aria-ops-core) — Pass 1
3. Every MPB-generated pak bundles `aria-ops-core-8.0.0.jar` in `lib/` — Pass 17

**Decompose VCF-CF's `vcfcf-adapter-base.jar` along the same SPI**:
- `Tester` — `boolean test(...)`
- `Discoverer` — discover resources
- `LiveCollector` — `getCurrentMetrics()`, `getEvents()`, `getRelationships()` (three-axis collection split)
- `HistoricalCollector` (optional) — for backfill / time-bucketed historical data

This works for BOTH Tier 1 (MPB-generated paks inherit this SPI via the bundled aria-ops-core) AND Tier 2 (hand-generated native adapters). Unified architecture.
