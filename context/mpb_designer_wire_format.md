...
In the shipped BuilderFile, expressions are compiled to **string templates**:

```
"${@@@MPB_QUOTE_BODY <jsonpath> @@@MPB_QUOTE}"
```

- Wrapping syntax: `${...}` for substitution
- Marker tokens: `@@@MPB_QUOTE_BODY ... @@@MPB_QUOTE` delimit the body-vs-non-body source
- `@@@MPB_QUOTE_BODY data.* @@@MPB_QUOTE` — navigate to `data` then iterate (list expression for `listExpression` field on metric requests)
- `@@@MPB_QUOTE_BODY type @@@MPB_QUOTE` — extract `type` field from current response item

Other namespaces (from designer JSON):
...
- `${requestParameters.<key>}` — chained-request substitution value

The runtime's `ResourceQueryHelperKt` + `HttpRequestHandlerKt` parse and evaluate these.
  ...
  "expressionParts": [
    {
      ...
      "originId": "<requestId>-<listId>-<fieldname>", // ATTRIBUTE form
      "originType": "ATTRIBUTE",                      // {ATTRIBUTE, PARAMETER, ARIA_OPS_METRIC}
...
`originType` values observed:
- `ATTRIBUTE` — extracted from a response dataModelList field
- `PARAMETER` — from a chaining param (URL substitution)
- `ARIA_OPS_METRIC` — from a foreign Aria Operations resource (cross-MP)
...
- generated resource kind: `mpb_ubiquiti_unifi_unifi___clients` (spaces → `___`, lowercased)

Tier 1 generators must mirror this slugification or accept user-provided ResourceKind keys with caveats about uniqueness.
...
1. **Document the precise dataModelList → expression-string compilation algorithm.** The designer captures structured expressions; the build-time compiler produces `${@@@MPB_QUOTE_BODY <path> @@@MPB_QUOTE}` strings. The exact mapping (regex inclusion, originType handling, multi-part templates) needs reverse-engineering. Probably in `BuilderQueryJsonNodeParserKt` / `BuilderQueryRequestParamParserKt` in the runtime jar.
