...
skeptical correctness/regression gate on `vcfops_*/` Python changes
...
The reviewer is spawned after `tooling` reports a `vcfops_*/` change and
**before** the PR is opened (CLAUDE.md delegation step 9 + "After tooling
changes"). Scope is **blanket** — every `vcfops_*/` diff. A
